...
bool ImGui::TabButton(const char* label, const char* icon, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ...
    RenderNavHighlight(bb, id);
    if (selected)
        RenderFrame(bb.Min, bb.Max, ImColor(119,4,4), true, style.FrameRounding);
    else
        RenderFrame(bb.Min, bb.Max, ImColor(39,39,39), true, style.FrameRounding);
    ...
    {
        PushFont(icons);
        ImGui::GetWindowDrawList()->AddText(icons, 16, ImVec2(bb.Min.x + 23, bb.Max.y + style.FramePadding.y - 55), ImGui::GetColorU32(ImGuiCol_Text), icon);
        PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    static std::map <ImGuiID, checkbox> animate;
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(88, 13, 13), false, 9.0f);

    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, ImColor(36, 5, 5), 30);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags, bool multi)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(-1, 15));
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 54, label_size.y + style.FramePadding.y * 4.0f));
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? 17 * val * 2 : 0.f, g.IO.DeltaTime * 8.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 21, bb.Max.y), ImColor(45, 45, 45), style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(129, 120, 132)));
    RenderTextClipped(bb.Min + ImVec2(9, 6), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    window->DrawList->AddText(ImVec2(bb.Max.x - w - 51, bb.Min.y - 20), GetColorU32(ImGuiCol_Text), label);
    PopStyleColor();

    PushFont(icons);
    RenderTextClipped(bb.Min + ImVec2(w + 25, 6), ImVec2(value_x2 + 20, bb.Max.y + 5), "D", NULL, NULL);
    PopFont();
    ...
    PushStyleColor(ImGuiCol_WindowBg, ImVec4(ImColor(45, 45, 45)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(4,0), ImColor(88, 13, 13), 5.0f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 4, grab_bb.Min.y + 1), 6.0f, ImColor(0, 0, 0), 30);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(129, 120, 132))); {
        RenderText(total_bb.Min, label);
        RenderTextClipped(total_bb.Min, total_bb.Max - ImVec2(5,0), value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
    } PopStyleColor();
